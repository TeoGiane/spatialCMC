...
out_dir <- "output/sensitivity_lambda"
if(!dir.exists(out_dir)) {
  dir.create(out_dir)
}

# Select values for var_scaling parameter
lambdas <- seq(0.1, 0.5, by=0.05)

# Run simulation study
for (curr_lam in lambdas) {
  # generate hier_params
  mix_params <- sprintf(mix_params_template, curr_lam)
  # Run CMC algorithm
  ...
  save(list=c("fit_cmc", "hier_params", "mix_params", "algo_params"),
       file=sprintf("%s/fit_cmc-lam%g.dat", out_dir, curr_lam))
}
