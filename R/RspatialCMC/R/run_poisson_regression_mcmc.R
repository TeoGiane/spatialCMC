...
#'
#' @export
pois_reg_mcmc <- function(data, offset, cov_matrix, geometry,
  ...
  POISREG_MCMC_EXE = Sys.getenv("POISREG_MCMC_EXE")
  if(POISREG_MCMC_EXE == ""){
    stop("POISREG_MCMC_EXE environment variable not set")
  }

  # Set-up template for run_poisreg_mcmc command
  params = paste('--data-file %s --hier-cov-file %s --adj-matrix-file %s',
        '--algo-params-file %s --hier-prior-file %s --mix-prior-file %s',
        '--chain-file %s')

  # Set run_mcmc command template
  RUN_CMD = paste(POISREG_MCMC_EXE, params)

  # Use temporary directory if out_dir is not set
  if(is.null(out_dir)) {
    rngstr <- paste0(sample(letters, 7, replace=T), collapse = "")
    out_dir = sprintf("%s/Rtmp-%s", dirname(POISREG_MCMC_EXE), rngstr); dir.create(out_dir, showWarnings = F)
    remove_out_dir = TRUE
  } else {
  ...
  CMD = sprintf(RUN_CMD, data_file, hier_cov_file, adj_matrix_file,
  ...
  if(errlog != 0L){
    ...
    errmsg <- "Something went wrong: 'run_poisreg_mcmc()' exit with status %d"
