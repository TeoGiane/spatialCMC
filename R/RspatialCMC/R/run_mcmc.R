...
#' \code{bnpdownscaling::BNPDownscalingState}. Such object store a generic iteration of the MCMC chain.
#'
#' @export
run_mcmc <- function(data, geometry, hier_type,
  ...
  params = paste("--data-file '%s' --adj-matrix-file '%s' --algo-params-file '%s'",
                 "--hier-type '%s' --hier-prior-file '%s' --mix-prior-file '%s' --chain-file '%s'")
  ...
  CMD = sprintf(RUN_CMD, data_file, adj_matrix_file, algo_params_file,
                hier_type, hier_params_file, mix_params_file, chain_file)

  # Execute run_mcmc_hrfix
  tryCatch({
    system(CMD)
  },
  error = function(e) {
    # Print error
    ERR_MSG <- "Failed with error: %s\n"
    message(sprintf(ERR_MSG, as.character(e)))
    # Clean up and return NULL
    unlink(paste0(out_dir,"/*.csv"))
    unlink(paste0(out_dir,"/*.recordio"))
