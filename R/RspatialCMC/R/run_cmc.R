...
run_cmc <- function(data, geometry = NULL, shard_allocation, algo_params,
  ...
  CMC_EXE = Sys.getenv("CMC_EXE")
  if(CMC_EXE == ""){
    stop("CMC_EXE environment variable not set")
  }
  ...
  if(is.null(out_dir)) {
    ...
    # out_dir = sprintf("%s/Rtmp-%s", dirname(CMC_EXE), rngstr); dir.create(out_dir, recursive = T, showWarnings = F)
    remove_out_dir = TRUE
  } else {
