...
plt_data <- ggplot() +
  geom_sf(data = sf_mun, aes(fill=data), col='gray25', linewidth=0.5, alpha=0.75) +
  geom_sf(data = geom_prov, color='darkred', fill=NA, linewidth=2) +
  scale_fill_gradient(low='steelblue', high='darkorange') +
  guides(fill = guide_colorbar("Y", position = "bottom", direction = "horizontal", barwidth=unit(2,"in"),
                               title.position="bottom", title.hjust = 0.5)) +
  theme_void()
plt_data
# pdf("output/PoisReg-plt_data.pdf", height = 4, width = 4); plt_data; dev.off()
...
df <- data.frame(1:nrow(beta_chain), beta_chain); names(df) <- c("Iter", "Beta1", "Beta2")
plt_beta1 <- ggplot(data = df) +
  geom_line(aes(x=Iter, y=Beta1)) +
  ylab(bquote(beta[1])) + theme(axis.title.x = element_blank())
plt_beta2 <- ggplot(data = df) +
  geom_line(aes(x=Iter, y=Beta2)) +
  ylab(bquote(beta[2])) + xlab("Iteration")
plt_betas <- gridExtra::grid.arrange(plt_beta1, plt_beta2)
