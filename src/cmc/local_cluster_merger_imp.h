...
// Constructor
template<class T>
LocalClusterMerger<T>::LocalClusterMerger(const std::vector<std::vector<bayesmix::AlgorithmState>> & _cluster_chains)
: cluster_chains(_cluster_chains) {
  // Set members
...
template<class T>
bayesmix::AlgorithmState LocalClusterMerger<T>::merge(size_t iter) {
  // std::cout << "LocalClusterMerger<T>::merge()" << std::endl;
  // Generate local clusters 
  auto local_clusters = generate_local_clusters(iter);

  // CHECK
  // std::cout << "Local Clusters before merging: " << std::endl;
  // for (auto && elem : local_clusters) { elem.print(); }
...
template<class T>
double LocalClusterMerger<T>::compute_qoi(bayesmix::AlgorithmState::ClusterState & lhs, bayesmix::AlgorithmState::ClusterState & rhs) {
  // Create buffer
  double out = 0;
...
std::deque<LocalCluster> LocalClusterMerger<T>::generate_local_clusters(const size_t & iter) {
  ...
  for (size_t it = 0; it < num_shards; it++) {
    ...
    // for (size_t c = 0; c < curr_state.cluster_states_size(); c++){
    //   std::cout << "Cluster label: " << c << std::endl;
    //   std::cout << "Local idx: " << std::endl;
    //   for (auto &&i : local_clust_idx[c]){std::cout << i << " ";} std::cout << std::endl;
    //   std::cout << "Global idx: " << std::endl;
    //   for (auto &&i : global_clust_idx[c]){std::cout << i << " ";} std::cout << std::endl;
    ...
    // std::cout << "Out of the for loop" << std::endl;
